...
class ExcelLibrary(object):
    ...
    def adjacent_cell_value(self, file_path, sheet_name, name):
        """Searches a Keyword from a Excel Sheet and Prints the value of the Adjacent Cell \n
        *Args:*\n
            _file_path_: Path of the Excel File \n
            _sheet_name_: Name of the Sheet \n
            _name_: Text to be Searched
            *Example:*\n
            | ${Adjacent_Cell}= | Adjacent Cell Value | file_path=path/to/file/file.xlsx | sheet_name=Sheet1 | name=${Search Term}
            """
        # Load the Excel file 
        file_path = str(file_path)
        workbook = openpyxl.load_workbook(filename=file_path)

        # Select the worksheet to search in 
        worksheet = workbook[sheet_name] 

        # Initialize flag to track if name was found 
        name_found = False

        # Initialize data to None 
        data = None

        # Loop through each row in column 1 and search for the name 
        for row in worksheet.iter_rows(min_row=2, min_col=1, max_col=1): 
            if row[0].value == name: 
                # Get the data from column 2 for the matching row 
                data = worksheet.cell(row=row[0].row, column=2).value 
                logger.info(f"The data for {name} is {data}.")
                name_found = True
                break 

        # If name was not found, set data to None 
        # if not name_found:
        #     logger.info(f"Could not find {name} in the Excel sheet.") 
        #     data = None
